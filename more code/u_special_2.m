function result=u_special_2(z,alpha)
...
[~,~,U] = svds(4*Dbar(N,0,f1,f2)^2 - alpha^2*W^2, 1, 'smallest');
UU = (2*(1/alpha)*Dbar(N,0,f1,f2) - W)*U;

ww = sparse(2*N+1, 2*N+1); ww(N+1,N+1) = 1;
Phi = Inv(2*Dbar(N,0,f1,f2)+kron(ww,ww)) * W * const_fun(N);

uu = F2Vect(UU,z,f1,f2,N);
phi = F2Vect(Phi,z,f1,f2,N);

result=exp(1i*alpha*imag(phi)) .* uu;
