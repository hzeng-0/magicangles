...
[~,v] = F2X(Up*Um*const_fun(N), 0,M, e1, e2);

sv = sqrt(-v); % physicist branch
sv = 1i*sv;
...
[zz,psi] = F2X(A,0,M,e1,e2);


Psi = interpolate(e1,e2,psi,M,real(z(:)'),imag(z(:)'));
Psi = reshape(Psi, size(z));
Psi = Psi + 0.5i*(a0' * z + a0 * conj(z));
