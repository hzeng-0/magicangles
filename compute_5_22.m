...
%% Movie of changing potential and changing magic angles

% -- PROLOGUE --
fig=figure;
vid=VideoWriter('.\results\changing_potential.mp4','MPEG-4'); open(vid)
set(fig, 'Units', 'Normalized', 'OuterPosition', [0 0 1 1]);

for lambda=0:0.02:1
% --------------

Upp = (1-lambda)*Up + lambda * Up2;
V = Upp * Upp.';
T = Inv(2*Dbar(N,0.1,f1,f2))^2 * V;
Alphas = 1./sqrt(eigs(T, 500));
Alphas = [Alphas; -Alphas];
[z, v] = F2X3(V*const_fun(N), 0, 300, e1, e2, 'nophaseplz');

clf(fig);
tl=tiledlayout(2,2,'tilespacing', 'compact');
title(tl, ['$\lambda = ' sprintf('%.2f', lambda) '$'], 'Interpreter', 'latex')
...
Wfun = @(z,w) 2i*K^2*(f(z,w,f1+f2) + om * f(z,w,f1) + om^2 * f(z,w,f2));



[z,w] = F2X3(W*const_fun(N), 0, 300, e1, e2, 'nophase');
figure
hold on
contourf(real(z), imag(z), abs(w), 32)
hex(zS); axis equal; colorbar
xlim([-0.63,0.63]), ylim([-0.63,0.63])
...
figure
contourf(real(z), imag(z), abs(Wz-w), 32)
axis equal
colorbar

%% Monodromy

[xx,yy]=meshgrid(-0.5:0.002:0.5, -0.5:0.002:0.5); z= xx+1i*yy;
[A0,A1,A2,A3] = cauchy_sol(z,conj(z),2);

figure
hold on
contourf(real(z), imag(z), log(abs(A0)), 32)
colorbar
axis equal
hex(zS)
...
figure
hold on
contourf(real(z), imag(z), (real(A0)), 32)
colorbar
axis equal
hex(zS)


%% Plot real spacing as function of epsilon

epsilons = 0:0.01:0.2;
realspacing = 0*epsilons;
realerrors = 0*epsilons;
imagspacing = 0*epsilons;
imagerrors = 0*epsilons;
for id=1:length(epsilons)
    disp(epsilons(id))
    % Upp = Up + epsilons(id) * Up2; V = Upp * Upp.' ;
    % V = Up*Um + epsilons(id)*W^2;
    % V = Up * Um + epsilons(id) * Up2 * Um2;
    Upp = Up + 0.1*epsilons(id)^2* W; V = Upp * Upp.';
    T = Inv(2*Dbar(N,0.1,f1,f2))^2 * V;
    Alphas = 1./sqrt(eigs(T, 300));
    Alphas = [Alphas; -Alphas];

    realAlphas = real(Alphas(abs(imag(Alphas)) < 0.01));
    realAlphas = sort(uniquetol(realAlphas, 0.01));
    realAlphas = realAlphas(realAlphas > 0);
    realdiff = diff(realAlphas); 
    realspacing(id) = realdiff(2); 
    realerrors(id) = abs(realdiff(2) - realdiff(3));

    imagAlphas = imag(Alphas(abs(real(Alphas)) < 0.01));
    imagAlphas = sort(uniquetol(imagAlphas, 0.01));
    imagAlphas = imagAlphas(imagAlphas > 0);
    imagdiff = diff(imagAlphas);
    imagspacing(id) = imagdiff(2); 
    imagerrors(id) = abs(imagdiff(2) - imagdiff(3));
end
%%
figure
hold on
plot(epsilons, real(realspacing))
plot(epsilons, realerrors)


%% Eigenfunction for perturbed potential (movie)

V = sym_potential(10,0,0) * sym_potential(10,0,0).' * const_fun(10);
M=300;
[z,v_pot] = F2X(V, 0, M, e1, e2);
r = 0.15;
v_pot = v_pot + 200*(bump(z,-1.5i/sqrt(3),r) + om*(bump(z,0.5*om',r) + bump(z,0.5*om'-om, r)) ...
    + om' * (bump(z, -0.5*om, r) + bump(z, -0.5*om+om', r)) );

% -- PROLOGUE --
fig=figure;
vid=VideoWriter('.\results\perturbed_potential_2.mp4','MPEG-4'); 
vid.FrameRate = 10;
open(vid)
set(fig, 'Units', 'Normalized', 'OuterPosition', [0 0 1 1]);

for alpha = 0.1:0.05:10
    % alpha = 10;
    clf(fig);
% --------------
...
[~,s,v] = svds(P, 1, 'smallest');
disp(diag(s))


w = v(:,1);
w = reshape(w, [M M]);
zz = z((1:2:M), (1:2:M)); w = w((1:2:M), (1:2:M));
...
hold on
title(['$\alpha = ' sprintf('%.2f', alpha), '$'], 'interpreter', 'latex')
% contourf(real(zz), imag(zz), abs(v_pot), 32)
contourf(real(zz), imag(zz), max(log(abs(w)), -7), 32)
hex(zS); axis equal; colorbar
...
function b = bump(z, z0, r)
    b = zeros(size(z)); % ensure same size and shape
    mask = abs(z - z0) < r;
    b(mask) = exp(-1 ./ (1 - (abs(z(mask) - z0).^2 / r^2)));
end
